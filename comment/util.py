...
class Util :
    ...
    def getNewVersion(fileName):
        try:
            with open('../comment/version.csv') as f:
        ...
        try:
            with open('../comment/version.csv', 'w', newline='') as f:
    ...
    @staticmethod
    def printTimeUsed(begin_time,end_time):
        h = int((end_time - begin_time) / (60 * 60))
        m = int((end_time - begin_time) / 60 - h * 60)
        s = int((end_time - begin_time) - h * 60 * 60 - m * 60)
        print("用时：" + str(h) + "h " + str(m) + "m " + str(s) + "s")
